    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input
          ...
          className="w-full max-w-xs px-3 py-2 bg-slate-900/50 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
        />
      ...
      {message && (
        <div
          className={`text-sm ${
            message.includes("success") ? "text-cyan-400" : "text-red-400"
          }`}
        >
          {message}
        </div>
      ...
      <div className="pt-2">
        <button
          type="submit"
          disabled={loading}
          className="px-4 py-2 text-sm font-medium text-black bg-cyan-500 rounded-lg hover:bg-cyan-500/90 disabled:opacity-50 disabled:cursor-not-allowed transition"
        >
